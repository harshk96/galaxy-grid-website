...
const ServiceDetail = ({ isDark }) => {
        ...
        const ctx = gsap.context(() => {
            ...
            sections.forEach(section => {
                ...
                if (revealItems.length > 0) {
                    // Set initial state to visible to prevent content from being hidden
            ...
            <div className={`h-screen flex items-center justify-center transition-colors duration-500 ${isDark ? 'bg-zinc-950 text-white' : 'bg-white text-zinc-900'}`}>
                <div className="flex flex-col items-center gap-6">
                    <div className="relative w-16 h-16">
                        <div className="absolute inset-0 border-4 border-[#ec2626]/20 rounded-full"></div>
                        <div className="absolute inset-0 border-4 border-[#ec2626] border-t-transparent rounded-full animate-spin"></div>
                    </div>
    ...
    if (!service) {
        return (
            <div className={`h-screen flex flex-col items-center justify-center gap-8 px-6 text-center transition-colors duration-500 ${isDark ? 'bg-zinc-950 text-white' : 'bg-white text-zinc-900'}`}>
                ...
                <Link to="/services" className="px-10 py-5 bg-[#ec2626] text-white rounded-full font-bold uppercase tracking-widest hover:bg-[#f9ab1c] transition-all shadow-xl shadow-[#ec2626]/20 active:scale-95">
    ...
    const accentText = "text-[#ec2626]";
    const accentBg = "bg-[#ec2626]";
    const borderClass = isDark ? "border-white/5" : "border-zinc-200";
    const cardBg = isDark ? "bg-white/[0.03]" : "bg-zinc-50";

    return (
        <div ref={containerRef} className={`min-h-screen selection:bg-[#ec2626] selection:text-white transition-colors duration-500 ${isDark ? 'bg-zinc-950 text-white' : 'bg-white text-zinc-900'}`}>

            {/* Hero Section */}
            <section id="hero" className="relative h-[90vh] md:h-screen w-full flex items-center overflow-visible px-6">
                ...
                <div className="max-w-7xl mx-auto w-full relative z-10">
                    ...
                    <h1 className="hero-reveal text-[13vw] md:text-[9vw] font-black uppercase leading-[0.85] tracking-tighter mb-10 max-w-[20ch]">
                    ...
                    <div className="hero-reveal flex flex-wrap gap-8">
                        <button className={`magnetic-btn px-10 py-5 ${accentBg} text-white rounded-full font-bold uppercase tracking-widest flex items-center gap-4 hover:scale-105 transition-transform duration-300 shadow-2xl shadow-[#ec2626]/30`}>
            ...
            <section className={`py-32 md:py-64 px-6 relative reveal-section overflow-hidden ${isDark ? 'bg-zinc-900/20' : 'bg-zinc-50/50'}`}>
                ...
                <div className="max-w-7xl mx-auto relative z-10">
                    <div className="text-center mb-24 md:mb-32 reveal-item">
                        ...
                        <div className="max-w-3xl mx-auto h-px bg-gradient-to-r from-transparent via-[#ec2626] to-transparent opacity-30"></div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                        {service?.features && service.features.length > 0 ? (
                            service.features.map((feature, i) => (
                                <div key={i} className={`reveal-item group p-8 md:p-10 rounded-[2rem] md:rounded-[2.5rem] border transition-all duration-700 hover:shadow-2xl flex flex-col items-start ${isDark ? 'bg-zinc-950/40 border-white/5 hover:border-[#ec2626]/30' : 'bg-white border-zinc-100 hover:border-[#ec2626]/30 shadow-lg'}`} style={{opacity: 1, transform: 'translateY(0px)'}}>
                                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 transition-all duration-500 group-hover:scale-110 group-hover:rotate-6 ${isDark ? 'bg-[#ec2626]/10 text-[#f9ab1c] border border-[#ec2626]/20' : 'bg-[#ec2626]/100 text-[#ec2626] border border-[#ec2626]/100'}`}>
                                        {feature.icon ? <feature.icon size={28} /> : <Zap size={28} />}
                                    </div>
                                    <h3 className="text-2xl md:text-3xl font-bold uppercase tracking-tight mb-4 group-hover:text-[#ec2626] transition-colors">{feature.title}</h3>
                        ...
                        {[1, 2, 3].map((set) => (
                            <div key={set} className="flex gap-16 animate-marquee shrink-0">
                                {service.technologies.map((tech, i) => (
                                    <span key={i} className="text-5xl md:text-7xl font-black uppercase tracking-tighter opacity-10 hover:opacity-100 hover:text-[#ec2626] transition-all duration-500 cursor-default flex items-center gap-16">
                                        {tech} <span className="w-4 h-4 rounded-full bg-[#ec2626]/20"></span>
                                    </span>
                                ))}
                            </div>
                        ))}
            ...
            <section className="py-32 md:py-64 px-6 reveal-section">
                <div className="max-w-7xl mx-auto">
                    <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-32 gap-12 reveal-item">
                        ...
                        <p className="max-w-md text-xl md:text-2xl font-light opacity-50 italic border-l-4 border-[#ec2626]/30 pl-8 pb-4">
                    ...
                    <div className="space-y-6">
                        {service?.process && service.process.length > 0 ? (
                            service.process.map((step, i) => (
                                <div key={i} className={`reveal-item group grid grid-cols-1 lg:grid-cols-12 gap-10 items-center p-10 md:p-14 rounded-[3.5rem] border transition-all duration-700 ${isDark ? 'border-white/5 hover:bg-white/[0.04]' : 'border-zinc-100 hover:bg-zinc-50 shadow-sm'}`} style={{opacity: 1, transform: 'translateY(0px)'}}>
                                    <div className="lg:col-span-1 border-b lg:border-b-0 lg:border-r border-current/10 pb-6 lg:pb-0 lg:pr-10">
                                        <div className="text-6xl font-black opacity-10 group-hover:opacity-100 group-hover:text-[#ec2626] transition-all duration-500">
                                            0{i + 1}
                                        </div>
                                    </div>
                            ...
                            <div className="text-center py-16">
                                <p className="text-xl opacity-60">No process steps available for this service.</p>
                            </div>
                        )}
                    </div>
                </div>
            ...
            <section className={`py-32 px-6 border-t ${borderClass}`}>
                <div className="max-w-7xl mx-auto">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-32 reveal-section">
                        {prevService && (
                            <Link to={`/services/${prevService.id}`} className={`reveal-item group relative h-[500px] rounded-[4rem] border ${borderClass} overflow-hidden p-14 flex flex-col justify-between transition-all duration-700 hover:border-[#ec2626]/40 shadow-xl`}>
                        ...
                        {nextService && (
                            <Link to={`/services/${nextService.id}`} className={`reveal-item group relative h-[500px] rounded-[4rem] border ${borderClass} overflow-hidden p-14 flex flex-col justify-between items-end text-right transition-all duration-700 hover:border-[#ec2626]/40 shadow-xl`}>
                                <div className="absolute inset-0 z-0 opacity-0 group-hover:opacity-20 transition-opacity duration-1000 grayscale group-hover:grayscale-0">
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-12 text-[10px] font-bold uppercase tracking-[0.5em] opacity-30 reveal-section">
                        <Link to="/services" className="reveal-item flex items-center gap-4 hover:text-[#ec2626] transition-colors py-4">
