    ...
    return (
        <div ref={containerRef} className="fixed inset-0 z-[60] pointer-events-none">
            ...
            >
                <button
                    onClick={onClose}
                    className={`self-end text-sm font-bold uppercase tracking-widest transition-colors mb-16 sm:mb-20 ${isDark ? 'hover:text-[#ec2626]' : 'hover:text-[#ec2626]'}`}
                >
                    Close
                </button>
                ...
                <div className="flex flex-col gap-6 sm:gap-8">
                    {links.map((link) => (
                        <a
                            key={link.id}
                            href={link.path}
                            className={`nav-link text-2xl sm:text-3xl md:text-4xl font-bold uppercase transition-colors cursor-pointer ${isDark ? 'hover:text-[#ec2626]' : 'hover:text-[#ec2626]'}`}
                ...
                <div className={`mt-auto pt-8 sm:pt-10 border-t ${isDark ? 'border-zinc-800' : 'border-zinc-200'}`}>
                    ...
                    <a href="mailto:info@galaxygrid.com" className={`block text-lg sm:text-xl transition-colors ${isDark ? 'hover:text-[#ec2626]' : 'hover:text-[#ec2626]'}`}>info@galaxygrid.com</a>
                </div>
            </div>
        </div>
    );
